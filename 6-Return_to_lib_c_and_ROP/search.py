...
class GadgetSearch(object):
    ...
    def get_format_count(self, gadget_format: str) -> int:
        ...
        Examples:
            ...
            => 2
        """
        return len([x[1] for x in string.Formatter().parse(gadget_format) if x[1] is not None])
    ...
    def get_register_combos(self, nregs: int, registers: Tuple[str]) -> Iterable[Iterable[str]]:
        ...
        Example:
            ...
            => [['eax', 'eax'],
                ...
                ['ebx', 'ebx']]
        """
        return [prod for prod in itertools.product(list(registers), repeat=nregs)]
    ...
    def format_all_gadgets(self, gadget_format: str, registers: Tuple[str]) -> Iterable[str]:
        ...
        register_combos = self.get_register_combos(nregs, registers)
        all_gadgets = []
        for comb in register_combos:
            gadget = gadget_format.format(*comb)
            all_gadgets += [gadget]
        return all_gadgets
    ...
    def find_all(self, gadget: str) -> Iterable[int]:
        ...
        res = []
        gadget_str = gadget + '; RET'
        gadget_bytes = assemble.assemble_data(gadget_str)

        found = self.dump.find(gadget_bytes)
        while found != -1:
            res.append(found + int(self.start_addr))
            found = self.dump.find(gadget_bytes, found + 1)
        return res
    ...
    def find_all_formats(self, gadget_format: str,
        ...
        for gadget in all_gadgets:
            all_addresses = self.find_all(gadget)
            if len(all_addresses) != 0:
                for addr in all_addresses:
                    sub_res = tuple((gadget, addr))
                    res.append(sub_res)
        return res
